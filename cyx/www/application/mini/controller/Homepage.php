...
class Homepage extends Common{
    ...
    public function buyNow(){
        ...
        $data = [
            ...
            // 'price' => $total_price,
            'price' => 0.01,
            'openid' => 'obC7r4h35bMX9qcpTubW6XBE6tzs',
        ];
        $wechatpay = new \app\common\Pay\WechatPay();
        $payParams = $wechatpay->pay($data);
        $apiRes = [
            'code' => 0,
            'data' => $payParams,
            'msg' => 'success'
    ...
    public function setuserinfo(){
        ...
        $phoneNo = getparameter('phoneNo');
        $invitationCode = getparameter('invitationCode');
    ...
    public function getProductListByCat(){
        ...
        $attr_id = getparameter('attr_id');
        $name = getparameter('name');
        $isOnlyThumb = getparameter('isOnlyThumbpicture');
        $sales = getparameter('sales');
    ...
    public function payNow(){
        $user_id = $this->getuserid();
        $product_list = object2array(json_decode(getparameter('product_list')));//商品信息列表
        $total_price = getparameter('total_price');//总价
        $userinfo = object2array(json_decode(getparameter('userinfo')));//用户信息
        $address_id = getparameter('address_id');//地址ID
        $freight = getparameter('freight');//运费
        $commission_used = getparameter('commission_used');//当次佣金
        $dividend_used = getparameter('dividend_used');//当次积分
        $manual_integral_used = getparameter('manual_integral_used');//当次手动添加的购物券
        $product_price = getparameter('product_price');//当次商品总价
        $note = getparameter('note');//备注
    ...
    public function entrySettlement(){
        $user_id = $this->getuserid();
        $productlist = object2array(json_decode(getparameter('productlist')));//商品信息列表
        // 启动事务
        \think\Db::startTrans();
        $tablePrefix = config("database.prefix");
        $msg = [];
    ...
    public function shopcartDel(){
        $user_id = $this->getuserid();
        $shopcartids = object2array(json_decode(getparameter('shopcartids')));
        // 启动事务
        \think\Db::startTrans();
        $tablePrefix = config("database.prefix");
        try{   
