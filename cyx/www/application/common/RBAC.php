...
class RBAC {
    ...
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = session('uid');
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if(config('USER_AUTH_TYPE') !=2 && !session('uid') )
            $seauthid	=	self::getAccessList($authId);
            session('_ACCESS_LIST',$seauthid);
        return ;
    }
    ...
    static function checkAccess() {
        ...
        if( config('USER_AUTH_ON') ){
            ...
            //检查当前模块是否需要认证
            if((!empty($_module['no']) && !in_array(strtoupper(request()->module()),$_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(request()->module()),$_module['yes']))) {
        ...
        if(self::checkAccess()) {
            ...
            if(!session('?adminid')) {
                if(config('USER_AUTH_TYPE')==2) {
                    ...
                    //通过数据库进行访问检查
                    $accessList = self::getAccessList(session('uid'));
                }else {
