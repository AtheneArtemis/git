...
//         dump($columnmap);
    ...
    /***---------------用户信息处理-------------------***/
    //获取账号信息
    function obtainuserinfo($uid){
        $list = model('User')->relation('company,province,city,zone,roles')->where('id',$uid)->find();
        if(!empty($list["picture"]))  $list["picture"] = uploadimgurl().$list["picture"];
        else $list["picture"] = uploadimgurl().config('persondefaultimg');
        if(!empty($list['company']["picture"]))  $list['company']["picture"] = uploadimgurl().$list['company']["picture"];
        else $list['company']["picture"] = uploadimgurl().config('companydefaultimg');
        return $list;
    }
    //获取公司信息
    function obtaincompanyinfo($companyid){
        $clist = model('company')->relation('province,city,zone')->find($companyid);
        if(!empty($clist["picture"]))  $clist["picture"] = uploadimgurl().$clist["picture"];
        else $clist["picture"] = uploadimgurl().config('companydefaultimg');
        return $clist;
    }
    //获取银行信息
    function obtainbankinfo($tablename,$primarykey){
        $bankslist = db('bankaccount')->where('objecttable',$tablename)->where('objectprimarykey',$primarykey)->where('status','gt','0')->select();
        $banklist = $bankslist[0];
        return $banklist;
    }
    //获取认证状态
    function obtainwhetheridentifiedstatus($uid){
        $whetheridentifiedid = $this->obtainonecolumnbyid('user', 'whetheridentified_id', $uid);
        $whetheridentifiedstatus = $this->obtaincolumn('enumeration', 'itemvalue', 'itemname',$whetheridentifiedid);
        return $whetheridentifiedstatus;
    }
    //获取认证状态
    function getwhetheridentifiedstatusinfo($uid){
        $whetheridentifiedid = $this->obtainonecolumnbyid('user', 'whetheridentified_id', $uid);
        $identifiedstatusinfo["identifiedstatusid"] = $whetheridentifiedid;
        $whetheridentifiedstatus = $this->obtaincolumn('enumeration', 'itemvalue', 'itemname',$whetheridentifiedid);
        $identifiedstatusinfo["identifiedstatus"] = $whetheridentifiedstatus;
        if(strcmp($whetheridentifiedid,"whetheridentified") === 0) {
            $displaycolor = "blue";
        } else if(strcmp($whetheridentifiedid,"ongoing") === 0) {
            $displaycolor = "green";
        } else {
            $displaycolor = "#ffffff";
            $identifiedstatusinfo["bgcolor"] = '#a0a0a0';
        }
        $identifiedstatusinfo["color"] = $displaycolor;
        return $identifiedstatusinfo;
    }
    //获取认证信息
    function obtainwhetheridentifiedinfo($model,$id){
        if (strcmp($model, 'companyidentify') ===0){
            $lists = db($model)->where('company_id',$id)->where('status','gt','0')->order('createtime desc')->select();
            if (!empty($lists[0]['masterpersonidcard1'])) $lists[0]['masterpersonidcard1'] = uploadimgurl().$lists[0]['masterpersonidcard1'];
            if (!empty($lists[0]['masterpersonidcard2'])) $lists[0]['masterpersonidcard2'] = uploadimgurl().$lists[0]['masterpersonidcard2'];
            if (!empty($lists[0]['companylisencecard'])) $lists[0]['companylisencecard'] = uploadimgurl().$lists[0]['companylisencecard'];
            
        }elseif (strcmp($model, 'professionalcertificate') ===0){
            $lists = db($model)->where('userperson_id',$id)->where('status','gt','0')->order('createtime desc')->select();
            
        }else{
            $lists = db($model)->where('userperson_id',$id)->where('status','gt','0')->order('createtime desc')->select();
            if (!empty($lists[0]['personidcard1'])) $lists[0]['personidcard1'] = uploadimgurl().$lists[0]['personidcard1'];
            if (!empty($lists[0]['personidcard2'])) $lists[0]['personidcard2'] = uploadimgurl().$lists[0]['personidcard2'];
            if (!empty($lists[0]['personidcardwithperson'])) $lists[0]['personidcardwithperson'] = uploadimgurl().$lists[0]['personidcardwithperson'];
    ...
    //公司组织结构
    //返回总经销商下属分销商的id,以数组方式返回
    function getdistributoroffirstdistributor($firstdistributorid) {
        $companytypeid = gettablecolumnvaluebyid("company","companytype_id",$firstdistributorid);
        if(strcmp($companytypeid,"firstleveldistributor") !== 0) {
            return null;
        }
        $distributorids = array();
        $model = db("Company");
        $map["status"] = array("gt","0");
        $map["uplevelcompany_id"] = array("eq",$firstdistributorid);
        $list = $model->where($map)->field("id")->select();
        foreach ($list as $key => $value){
            $distributorids[$key] = $value['id'];
        }
        return $distributorids;
    }
    
    //返回水电工所属上级公司
    function getcompanyidofelectrician($electricianuserid) {
        $euserinfo = $this->obtaincolumnbyid('user','id,isshop,company_id',$electricianuserid);
        if ($euserinfo['isshop'] == 1){
            $uplevelcompanyid = $this->obtainonecolumnbyid('company','uplevelcompany_id',$euserinfo['company_id']);
            $uplevelcompanyinfo = $this->obtaincolumnbyid('company','id,name',$uplevelcompanyid);
        }else{
            $uplevelcompanyinfo = $this->obtaincolumnbyid('company','id,name',$euserinfo['company_id']);
        }
        return $uplevelcompanyinfo;
    }
    
    //返回水电工所属总经销商
    function getfirstdistributoridofelectrician($electricianuserid) {
        $companyid = gettablecolumnvaluebyid("user","company_id",$electricianuserid);
        $companytypeid = gettablecolumnvaluebyid("company","companytype_id",$companyid);
        if(strcmp($companytypeid,"firstleveldistributor") === 0) {
            return $companyid;
        } else if(strcmp($companytypeid,"distributor") === 0) {
            $companyid = gettablecolumnvaluebyid("company","uplevelcompany_id",$companyid);
            return $companyid;
        } else {
            return null;
        }
    ...
    //返回网点所属上级公司
    function getcompanyidforshop($shopcompanyid) {
        $companyid = gettablecolumnvaluebyid("company","uplevelcompany_id",$shopcompanyid);
        return $companyid;
    }
    //返回网点所属总经销商
    function getfirstdistributoridforshop($shopcompanyid) {
        $companyid = gettablecolumnvaluebyid("company","uplevelcompany_id",$shopcompanyid);
        $companytypeid = gettablecolumnvaluebyid("company","companytype_id",$companyid);
        if(strcmp($companytypeid,"firstleveldistributor") === 0) {
            return $companyid;
