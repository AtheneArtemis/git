...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		...
		S($cachename,$value,$expired); // add by yangyong for thinkphp implementation
		return true;
	}
	...
	protected function getCache($cachename){
		...
		$data = S($cachename);			// add by yangyong for thinkphp implementation
		return $data;
	}
	...
	protected function removeCache($cachename){
		...
		S($cachename,NULL);	// add by yangyong for thinkphp implementation
		return true;
	}
...
// 	    fdbg('--统计返回数据');fdbg($result);
	    if ($result)
	    {
	        $json = json_decode($result,true);
// 	        fdbg('--统计返回转义数据');fdbg($json);
	        if (!$json || !empty($json['errcode'])) {
	            $this->errCode = $json['errcode'];
	            $this->errMsg = $json['errmsg'];
	            return false;
